...
The pipeline combines deterministic rules, spaCy/Presidio named-entity recognition, and an optional LLM second pass. It can redact detected PHI as typed masks, deterministic hash tokens, or realistic surrogate values. It also includes a small FHIR ingestion demo that extracts text from synthetic Synthea FHIR Bundles before de-identification.
...
Extract and de-identify a synthetic FHIR Bundle:

```bash
uv run python -m phi.ingest.fhir data/fhir/Stanley_Balistreri_08ce8462-f56a-4fd6-b986-ea75c5c6edb2.json --strategy mask
```
...
The repository includes three generated Synthea FHIR Bundles in `data/fhir/`. These are synthetic patient records shaped like real FHIR JSON exports. They are used to show where the de-identifier fits in a more realistic clinical data flow:

```text
Synthetic FHIR Bundle
  -> extract patient/contact/identifier fields and clinical resource summaries
  -> normalize those fields into readable clinical text
  -> run PHI detection and redaction
  -> return redacted text plus audit counts
```

Run extraction only:

```bash
uv run python -m phi.ingest.fhir data/fhir/Stanley_Balistreri_08ce8462-f56a-4fd6-b986-ea75c5c6edb2.json --extract-only
```

Run extraction plus redaction:

```bash
uv run python -m phi.ingest.fhir data/fhir/Stanley_Balistreri_08ce8462-f56a-4fd6-b986-ea75c5c6edb2.json --strategy mask
```

The FHIR adapter is intentionally small. It does not try to be a full FHIR server, validator, or ETL framework. Its purpose is to demonstrate the upstream workflow: structured clinical records are normalized into text, then the same PHI de-identification engine is applied.
...
```text
Input note or extracted FHIR text
  -> rule recognizers
  -> spaCy/Presidio NER
...
The repository includes 30 labelled synthetic notes in `data/notes/` and gold labels in `data/labels.json`. Those notes are generated by `scripts/generate_corpus.py` so each PHI span has known character offsets for scoring.

The Synthea FHIR examples in `data/fhir/` are separate workflow fixtures. They demonstrate realistic FHIR-shaped input, but they are not part of the precision/recall benchmark because they do not include gold span labels.
...
| --- | --- | --- |
| Identifiers | 1.000 / 1.000 / 1.000 | 1.000 / 1.000 / 1.000 |
| Overall | 0.985 / 0.990 / 0.987 | 0.965 / 0.970 / 0.967 |
...
| `POSTCODE` | NZ/AU 4-digit postcodes and US ZIP codes when address context is present |
...
src/phi/
  ...
  ingest/
    fhir.py              Synthetic FHIR Bundle text extraction

data/
  fhir/                  Generated Synthea FHIR Bundles
...
- The Synthea FHIR examples are workflow fixtures, not a validated benchmark.
